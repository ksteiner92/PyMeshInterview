...
static constexpr
    typename build_indices<std::tuple_size<Bare<TupleType>>::value>::type
    make_indices() {
  return {};
}

template <typename TupleType, int... Indices>
std::array<typename std::tuple_element<0, Bare<TupleType>>::type,
           std::tuple_size<Bare<TupleType>>::value>
to_array(TupleType &&tuple, indices<Indices...>) {
  using std::get;
  return {{get<Indices>(std::forward<TupleType>(tuple))...}};
}

template <typename TupleType>
auto to_array(TupleType &&tuple)
    -> decltype(to_array(std::declval<TupleType>(),
                         make_indices<TupleType>())) {
  return to_array(std::forward<TupleType>(tuple), make_indices<TupleType>());
}

template <typename... Tuple> struct TupleEqual {
  static constexpr size_t ndim = sizeof...(Tuple);

  bool operator()(const std::tuple<Tuple...> &t1,
                  const std::tuple<Tuple...> &t2) const noexcept {
    ...
    do {
      ...
      for (size_t idim = 0; idim < ndim; ++idim)
        perm_res =
            perm_res && t1_array[indices1[idim]] == t2_array[indices2[idim]];
      res = res || perm_res;
...
template <typename T, std::size_t N> struct ArrayNonAssociativeEqual {
  bool operator()(const std::array<T, N> &a,
    ...
    if (std::lexicographical_compare(b.begin(), b.end(), a.begin(), a.end()))
      do {
      } while (!(res = (res || a == comp)) &&
               std::next_permutation(comp.begin(), comp.end()));
    else
      do {
      } while (!(res = (res || a == comp)) &&
               std::prev_permutation(comp.begin(), comp.end()));
    return res;
  }
};

template <typename... T> struct TupleEqual2 {
  template <std::size_t I0, std::size_t... I>
  static constexpr bool equal(const std::tuple<T...> &t1,
                              const std::tuple<T...> &t2) noexcept {
    return t1 == t2 || TupleEqual2<T...>::equal(std::tie(), std::tie());
  }

  constexpr bool operator()(const std::tuple<T...> &t1,
                            const std::tuple<T...> &t2) const noexcept {
    return t1 == t2 || true;
  }
};

template <typename T> struct TupleEqual2<T> {
  static constexpr bool equal(const std::tuple<T> &t1,
                              const std::tuple<T> &t2) noexcept {
...
template <typename T> struct ArrayHash<T, 1> {
  std::size_t operator()(const std::array<T, 1> &key) const noexcept {
    return key[0];
  }
};

template <typename T> struct ArrayHash<T, 2> {
...
template <typename T> struct ArrayHash<T, 3> {
...
template <class T, std::size_t N> struct ArrayHash {
  ArrayHash() = default;
...
template <typename... I> struct TupleHash {
  ...
  operator()(const std::tuple<I...> &key) const noexcept {
    ...
    // result ^= std::hash<std::size_t>{}(args) + 0x9e3779b9 + (result << 6) +
    // (result >> 2)
    std::apply(
        [&result](auto &&... args) {
          ((result ^= std::hash<std::size_t>{}(args)), ...);
        },
        key);
    return result;
  }
...
template <typename T, std::size_t N> struct generate_hash_type {
