...
template <uint Dim>
Mesh<Dim, 0>::Mesh()
    : coordinates_owner(std::make_unique<std::vector<double>>()),
      vertices_container(this),
      vertices_proxy(std::make_unique<VerticesProxy>(this)) {
...
Mesh<Dim, 0>::Mesh(Mesh<Dim, TopDim> *mesh)
    : vertices_container(mesh->vertices_container),
      vertices_proxy(std::make_unique<VerticesProxy>(this)) {
...
Mesh<Dim, 0>::VerticesProxy::create(const std::vector<ID> &vertices) {
  ...
  if constexpr (Dim > 0) {
    if (id >= mesh->coordinates->size() / Dim) {
      throw std::out_of_range("No point exists for this ID");
    }
  }
...
template <uint Dim>
typename Mesh<Dim, 0>::VerticesProxy &Mesh<Dim, 0>::VerticesProxy::add(
    const EigenDRef<const Eigen::MatrixXd> &points) {
  if (points.cols() != Dim)
    throw std::logic_error("A vertex consists of one point only");
  for (size_t row = 0; row < points.rows(); ++row) {
    ...
    ID id;
    if constexpr (Dim > 0) {
      id = mesh->coordinates->size() / Dim - 1;
    } else {
      id = mesh->coordinates->size() - 1;
    }
    create({id});
  }
  return *this;
...
template <uint Dim>
MeshElement *Mesh<Dim, 1>::EdgesProxy::create(const std::vector<ID> &indices) {
  if (indices.size() != 2) {
    throw std::logic_error("An edge consists of two point only");
  }
  return mesh->edges_container.insert(indices[0], indices[1]);
}

template <uint Dim>
const std::vector<const MeshElement *> *
Mesh<Dim, 1>::EdgesProxy::edgesOfVertex(ID vid) const {
  return mesh->edges_container.elementsOfVertex(vid);
}
...
template <uint Dim>
MeshElement *Mesh<Dim, 2>::FacesProxy::create(const std::vector<ID> &indices) {
  if (indices.size() != 3)
    throw std::logic_error("A face consists of 3 points only");
...
Cell *Mesh<3, 3>::getCell(size_t idx) const {
  if (idx >= cells.size()) {
    std::cout << "Mesh3D getCell: Index out of bounds" << std::endl;
    throw std::out_of_range("Cell index out of range");
  }
  return cells[idx].get();
}

Cell *Mesh<3, 3>::getCellByID(ID id) const { return nullptr; }

size_t Mesh<3, 3>::getNumCells() const { return cells.size(); }
