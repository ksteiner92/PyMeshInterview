...
public:
  ...
  public:
    ...
    ConstIterator(ConstMeshElementsProxy *container, std::size_t pos)
    ...
    pointer operator->() { return (*container)[pos]; }
  ...
  explicit ConstMeshElementsProxy(
      std::vector<std::unique_ptr<MeshElement>> *elements,
      std::vector<ID> *ref = nullptr)
  ...
  ConstMeshElementsProxy &
  operator=(ConstMeshElementsProxy &&) noexcept = default;
...
public:
  ...
  public:
    ...
    iterator(MeshElementsProxy *container, std::size_t pos)
        : container(container), pos(pos) {}
  ...
  MeshElementsProxy &operator=(const MeshElementsProxy &) = delete;

  MeshElementsProxy(MeshElementsProxy &&) noexcept = default;

  MeshElementsProxy &operator=(MeshElementsProxy &&) noexcept = delete;
  ...
  MeshElementsProxy &add(const EigenDRef<const MatrixXid> &indices) {
    for (size_t irow = 0; irow < indices.rows(); ++irow) {
      ...
      for (std::size_t i = 0; i < indices.cols(); ++i)
        data.push_back(indices.row(irow)[i]);
      create(data);
    }
    return *this;
  }

  virtual MeshElementsProxy &
  getOrCreateFromFacets(const EigenDRef<const MatrixXid> &indices) {
    throw std::logic_error("Mesh element does not have facets");
  }

  virtual MeshElementsProxy &
  addFromFacets(const EigenDRef<const MatrixXid> &indices) {
    throw std::logic_error("Mesh element does not have facets");
  }

  virtual MeshElementsProxy &
  addFromRidges(const EigenDRef<const MatrixXid> &indices) {
    throw std::logic_error("Mesh element does not have ridges");
  }

  virtual MeshElementsProxy &
  addFromPeaks(const EigenDRef<const MatrixXid> &indices) {
...
template <uint Dim, uint TopDim = Dim> class Mesh {};

template <uint Dim> class Mesh<Dim, 0> : public MeshBase {
...
public:
  ...
  template <uint TopDim> explicit Mesh(Mesh<Dim, TopDim> *mesh);
...
template <uint Dim> class Mesh<Dim, 1> : public Mesh<Dim, 0> {
...
public:
  ...
  public:
    ...
    const std::vector<const MeshElement *> *edgesOfVertex(ID vid) const;

  private:
    Mesh<Dim, 1> *mesh;
    std::unordered_map<ID, std::vector<const MeshElement *>> vid2edges_;
  };
  ...
  std::vector<ID> getEdgesOfVertex(ID vid) const;
...
protected:
  ...
  // Neighbor relations
  // std::unique_ptr<std::unordered_map<std::array<ID, 2>, ID, ArrayHash<ID, 2>,
  // ArrayNonAssociativeEqual<ID, 2>>> vhash2eid_owner;
  // std::unordered_map<std::array<ID, 2>, ID, ArrayHash<ID, 2>,
  // ArrayNonAssociativeEqual<ID, 2>>* vhash2eid;
  // std::unique_ptr<std::unordered_multimap<ID, Edge<Dim>*>> vertex2edge_owner;
  // std::unordered_multimap<ID, Edge<Dim>*>* vertex2edge;
};

template <uint Dim> class Mesh<Dim, 2> : public Mesh<Dim, 1> {
...
protected:
  ...
  // Neighbor relations
  // std::unique_ptr<std::unordered_map<std::array<ID, 3>, ID, ArrayHash<ID, 3>,
  // ArrayNonAssociativeEqual<ID, 3>>> vhash2fid_owner;
  // std::unordered_map<std::array<ID, 3>, ID, ArrayHash<ID, 3>,
  // ArrayNonAssociativeEqual<ID, 3>>* vhash2fid;
...
template <> class Mesh<3, 3> : public Mesh<3, 2> {
public:
  Mesh();
...
template <uint Dim, uint TopDim> using Boundary = Mesh<Dim, TopDim - 1>;
