...
public:
  ...
  // virtual EigenDMap<const Eigen::VectorXd> getPoint(std::size_t idx) const =
  // 0;
...
template <uint Dim, uint SimplexDim> class SimplexBase : public MeshElement {
  static_assert(SimplexDim <= Dim,
                "Simplex dimension has to be smaller or equal than dimension");
  static_assert(Dim <= 3, "Dimension can only be 1, 2 or 3");

public:
  SimplexBase() = delete;

  SimplexBase(MeshBase *mesh, ID id,
              const std::array<ID, SimplexDim + 1> &vertices);
...
template <uint Dim, uint SimplexDim> class Simplex {};

template <uint Dim> class Simplex<Dim, 0> : public SimplexBase<Dim, 0> {
public:
  using SimplexBase<Dim, 0>::SimplexBase;
  Eigen::VectorXd center() const override;
};

template <uint Dim> class Simplex<Dim, 1> : public SimplexBase<Dim, 1> {
public:
  using SimplexBase<Dim, 1>::SimplexBase;
  Eigen::VectorXd center() const override;
};

template <uint Dim> class Simplex<Dim, 2> : public SimplexBase<Dim, 2> {
public:
  using SimplexBase<Dim, 2>::SimplexBase;
  Eigen::VectorXd center() const override;
};

template <uint Dim> class Simplex<Dim, 3> : public SimplexBase<Dim, 3> {
public:
  using SimplexBase<Dim, 3>::SimplexBase;
  Eigen::VectorXd center() const override;
};

template <uint Dim> using Vertex = Simplex<Dim, 0>;

template <uint Dim> using Edge = Simplex<Dim, 1>;

template <uint Dim> using Face = Simplex<Dim, 2>;
