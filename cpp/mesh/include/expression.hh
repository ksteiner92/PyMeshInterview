...
template <typename T>
using is_complex = std::disjunction<
    std::is_same<std::remove_reference_t<T>, std::complex<float>>,
...
template <typename T>
using is_number =
    std::disjunction<std::is_arithmetic<std::remove_reference_t<T>>,
                     is_complex<T>>;

template <typename T> static constexpr bool is_number_v = is_number<T>::value;
...
//std::enable_if_t<is_number_v<R>, Constant<R>> expr(T &&val) {
//  return Constant<R>(std::move(val));
//}
...
template <typename T> auto eval(const Expression<T> &expr);
...
template <typename T> struct Expression {
  ...
  Multiply<T, Right> operator*(const Expression<Right> &r) {
    return Multiply<T, Right>(std::move(*this),
                              std::move(const_cast<Expression<Right> &>(r)));
  }

  template <typename Right>
  Divide<T, Right> operator/(const Expression<Right> &r) {
    return Divide<T, Right>(std::move(*this),
                            std::move(const_cast<Expression<Right> &>(r)));
  }

  template <typename Right>
  Add<T, Right> operator+(const Expression<Right> &r) {
    return Add<T, Right>(std::move(*this),
                         std::move(const_cast<Expression<Right> &>(r)));
  }

  template <typename Right>
  Subtract<T, Right> operator-(const Expression<Right> &r) {
    return Subtract<T, Right>(std::move(*this),
                              std::move(const_cast<Expression<Right> &>(r)));
  }

  template <typename Right> Negate<T> operator-() {
...
struct BinaryOperator : public Expression<Result> {
  BinaryOperator(Expression<Left> &&l, Expression<Right> &&r)
      : l(std::forward<Expression<Left> &>(l)),
        r(std::forward<Expression<Right> &>(r)) {}

  std::size_t size() const override { return std::max(l.size(), r.size()); }

  Expression<Left> &l;
  Expression<Right> &r;
};

template <typename T> struct UnaryOperator : public Expression<T> {
  explicit UnaryOperator(Expression<T> &&expr) : expr(expr) {}

  std::size_t size() const override { return expr.size(); }

  Expression<T> &expr;
};

template <typename T> struct Negate : public UnaryOperator<T> {
  using super = UnaryOperator<T>;
  using super::UnaryOperator;

  T operator()(std::size_t i) const override { return -super::expr(i); }
};
...
template <typename T, typename Expr, template <typename> class Result>
using enable_if_expression = std::enable_if_t<
    std::is_base_of_v<Expression<T>, std::remove_reference_t<Expr>>,
    Result<std::remove_reference_t<Expr>>>;

template <typename Left, template <typename, typename> class Operator,
          typename Right,
          typename Result = decltype(std::declval<Operator>()(0))>
using enable_if_binary_expression = std::enable_if_t<
    std::conjunction_v<
        std::is_base_of<Expression<Result>, std::remove_reference_t<Left>>,
        std::is_base_of<Expression<Result>, std::remove_reference_t<Right>>,
        std::is_base_of<BinaryOperator<std::remove_reference_t<Left>,
                                       std::remove_reference_t<Right>, Result>,
                        Operator<std::remove_reference_t<Left>,
                                 std::remove_reference_t<Right>>>>,
    Operator<std::remove_reference_t<Left>, std::remove_reference_t<Right>>>;

template <typename T> struct Constant : public Expression<T> {
  explicit Constant(T &&value) : value(std::forward<T>(value)) {}

  T operator()(std::size_t i) const override { return value; }
...
template <typename T> struct Variable : public Expression<T> {
  explicit Variable(std::valarray<T> &&value)
      : value(std::forward<std::valarray<T>>(value)) {}

  explicit Variable(std::size_t s) : value(std::valarray<T>(s)) {}

  T operator()(std::size_t i) const override { return value[i]; }

  const std::valarray<T> &operator()() const { return value; }
...
/*template<typename Left, template<typename, typename> class Operator, typename
Right, typename Result = decltype(std::declval<Operator>()(0))> struct
BinaryExpression : public Expression<Result>
{
   BinaryExpression(Left&& l, Right&& r) : op(std::forward<Left>(l),
std::forward<Right>(r)) {}
...
/*template<typename Left, typename Right, typename L =
std::remove_reference_t<Left>, typename R = std::remove_reference_t<Right>>
enable_if_binary_expression<L, Multiply, R>
operator*(Left&& l, Right&& r)
{
   return enable_if_binary_expression<L, Multiply, R>(std::move(l),
std::move(r));
}
...
sq(Expr&& expr)
{
   return enable_if_binary_expression<Expr, Multiply, Expr>(std::move(expr),
std::move(expr));
}*/
